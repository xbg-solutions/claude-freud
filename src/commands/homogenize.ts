...
import type { Project, CfreudConfig, HomogenizeProposal, Promotion, PromotionCategory } from '../types.js'
...
interface HomogenizeApiResponse {
...
function buildGlobalClaudeMd(promotions: Promotion[], sourceProjects: string[], existing: string | null): string {
  const byCategory = (cat: PromotionCategory) =>
    promotions
...
export async function homogenizeCommand(
  projects: Project[],
  config: CfreudConfig,
  applyMode: boolean,
  mdMode: boolean
  ...
  const result = await analyseJson<HomogenizeApiResponse>(prompt, config)

  const proposal: HomogenizeProposal = {
