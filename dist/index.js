...
program
    ...
    try {
        ...
        if (!a) {
            console.error(chalk.red(`  Patient not found: ${projA}`));
            process.exit(1);
        }
        if (!b) {
            console.error(chalk.red(`  Patient not found: ${projB}`));
            process.exit(1);
        }
...
program
    ...
    try {
        ...
        const projects = project
            ? [await loadProject(config.claudeDir, project)].filter(Boolean)
            : await loadAllProjects(config.claudeDir);
        if (projects.length === 0) {
            console.log(chalk.dim('  No patients found to analyse.\n'));
            process.exit(0);
        }
        await homogenizeCommand(projects, config, opts.apply ?? false, opts.md ?? false);
    }
    catch (err) {
        console.error(chalk.red(`\n  ${err}\n`));
        process.exit(1);
